...
; Musica
   lda #1
   sta musicTimer       ; Começa o timer
   lda #0
   sta musicIndex       ; Começa na primeira nota
...
.drawBomb
   ...
   lda RED             ; 3         get the random number
...
MadBomberColors
   .byte BLACK+2,BLACK+2,WHITE,WHITE,WHITE,BLACK+10
   .byte RED+2,BLACK+10,RED+2,BLACK+10,RED+2,BLACK+10,RED+2
   .byte BLACK+10,RED+2,BLACK+10,RED+2,BLACK+10,RED+2,BLACK+15
   .byte BLACK+15,BLACK+15,BLACK+15,BLACK+15,BLACK+15
   .byte BLACK+15,BLACK+15,BLACK+15,BLACK+15,BLACK+10,RED+2,BLACK+10
   
GameColors
   .byte BLACK     ; Bomber area background color
   .byte BLUE+10     ; player area background color  (Esgoto)
   .byte $42  ; player1 score color (Vermelho escuro)
   .byte $42    ; player2 score color
   
BombColors
   ; --- LINHAS 1-7: Cordinha e Nó (BRANCO) ---
   .byte WHITE     ; 1. Espaço vazio (Transparente)
   .byte WHITE    ; 2. Ponta da corda
   .byte WHITE    ; 3. Corda
   .byte WHITE    ; 4. Corda
   .byte WHITE    ; 5. Corda
   .byte WHITE    ; 6. Curva
   .byte RED   ; 8. Base do balão 
   
   ; --- LINHAS 8-14: Corpo do Balão ---
   .byte RED      ; 10. Meio (Cor Pura)
   .byte RED      ; 10. Meio (Cor Pura)
   .byte RED      ; 10. Meio (Cor Pura)
   .byte RED      ; 11. Meio
   .byte RED      ; 12. Meio
   ...
   ; --- LINHAS 15-16: Espaço Vazio ---
   .byte $00      ; 15. Espaço
   .byte $00      ; 16. Espaço
   
BucketColors
   ; Faca 1
   .byte BLACK,BLACK,BLACK+6,WHITE,WHITE,WHITE,WHITE,WHITE
   .byte WHITE,RED,RED,RED,RED,RED,RED,RED  ; 
   ; Faca 2
   .byte BLACK,BLACK,BLACK+6,WHITE,WHITE,WHITE,WHITE,WHITE
   .byte WHITE,RED,RED,RED,RED,RED,RED,RED  ; 
   ; Faca 3
...
FallingBomb0 ;
   ...
   .byte $10 ; |...X....|
   .byte $10 ; |...X....|
   .byte $10 ; |...X....| (Nó do balão)
   .byte $18 ; |...XX...| (Base do balão)
   .byte $3C ; |..XXXX..|
   .byte $7E ; |.XXXXXX.|
   .byte $7E ; |.XXXXXX.| (Corpo do balão)
   .byte $7E ; |.XXXXXX.|
   .byte $3C ; |..XXXX..|
   .byte $18 ; |...XX...| (Topo)
   .byte $00 ; |........|
...
FallingBomb1 ; Frame 2: Corda centralizada (esticando)
   .byte $00 ; |........| 
   .byte $04 ; |.....X..| 
   .byte $04 ; |.....X..| 
   .byte $08 ; |....X...|
...
FallingBomb2 ; Frame 3: Corda para a direita
   .byte $00 ; |........| 
   .byte $04 ; |.....X..| 
   .byte $04 ; |.....X..| 
   .byte $08 ; |....X...|
...
FallingBomb3 ; Frame 4: Corda voltando (efeito cobrinha)
   .byte $00 ; |........| 
   .byte $04 ; |.....X..| 
   .byte $04 ; |.....X..| 
   .byte $08 ; |....X...|
...
PlayCircusMusic SUBROUTINE
    dec musicTimer          ; Diminui o tempo da nota atual
    bne .exitMusic          ; Se não chegou a zero, continua tocando a mesma nota

   ; leitura e loop
    ldx musicIndex          ; Carrega o índice atual
    lda CircusData,x        ; Lê a frequência da nota
    cmp #$FF                ; Verifica se é o fim da música (marcador $FF)
    bne .playNote
    lda #0                  ; Se for o fim, reinicia o índice
    sta musicIndex
    tax                     ; Atualiza X para 0
    lda CircusData,x        ; Carrega a primeira nota

; implementação do som
.playNote
    sta AUDF1               ; Define a frequência no Canal 1
    lda #12                 ; tom
    sta AUDC1               ; Controle de áudio Canal 1    
    lda #6                  ; volume
    sta AUDV1
    inx                     ; Avança para o byte de duração
    lda CircusData,x        ; Lê a duração
    sta musicTimer          ; Define o novo tempo
    inc musicIndex          ; Aponta para a próxima nota (Frequência)
    inc musicIndex          ; Aponta para a próxima nota (Duração)
...
CircusData:
   ; Formato: .byte Frequencia, Duracao (Frames)
    .byte 31, 60    ; Nota mais grave possivel
    .byte 30, 40    ; 
    .byte 31, 60    ; Volto pro grave
    .byte 28, 35    ;
    .byte 29, 35    ; Meio tom acima 
    .byte 28, 35    ; Volta
    .byte 25, 40    ; Sobe um pouco 
    .byte 30, 50    ; Grave sustentado
    .byte 31, 90    ; Som mais grave até o fim
    .byte $FF       ; Final da musica
   
   org ROM_BASE_ADDRESS+4096-4, 0   ; 2K ROM
